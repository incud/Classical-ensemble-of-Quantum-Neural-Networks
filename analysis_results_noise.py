...
def run():   
    ...
    #plot_error_layers_train(directory_experiment='executions/ibm/hardware_efficient/',dataset=f'{dataset}',title="Training error of each model in terms of the number of layers (noise)")
    plot_error_layers(directory_experiment='executions/ibm/hardware_efficient/',dataset=f'{dataset}',title="Experiment I (simulated noise)")
    plot_error_layers_comparison(directory_experiment='executions/jax/hardware_efficient/',directory_experiment_noise='executions/ibm/hardware_efficient/',dataset=f'{dataset}',title="Experiment I (ideal vs simulated noise)")
...
def get_experiment_predictions(directory_experiment,model):
    predictions = [np.load(f'{directory_experiment}/{model}/y_predict_{i}.npy') for i in range(5)]
    return np.array(predictions)
    
def get_experiment_predictions_noise(directory_experiment_noise,model):
    predictions = [np.load(f'{directory_experiment_noise}/{model}/y_predict_{i}_noise.npy') for i in range(5)]
    return np.array(predictions)
    
def get_experiment_predictions_train(directory_experiment,model):
    predictions = [np.load(f'{directory_experiment}/{m_noodel}/y_predict_train_{i}_noise.npy') for i in range(5)]
    return np.array(predictions)
...
def get_model_avg_error_noise(directory_experiment_noise,model):
    X_train,y_train,X_test,y_test = get_experiment_data(directory_experiment_noise)
    predictions = get_experiment_predictions_noise(directory_experiment_noise,model)
    scaler = load(f'{directory_experiment_noise}/scaler.pkl')
...
def get_model_errors_noise(directory_experiment_noise,dataset,model):
    errors = []
    for i in range(1,4):
        try:
            model_mean, model_std = get_model_avg_error_noise(directory_experiment_noise+f'{i}/{dataset}',model)
            errors.append(model_mean)
        except Exception:
            print(f'\n====== ERROR AT: {i} ======\n')
            pass        
...
def plot_error_layers_comparison(directory_experiment, directory_experiment_noise, dataset, title=f"Error of each model in terms of the number of layers"):
    MEDIUM_SIZE = 12
    ...
    x_ticks = ['full_model', 'bagging_feature08_sample02', 'bagging_feature08_sample10']
    markers = ['o', 'P', '8', 'x', 's', 'p']
    count = 0
    ...
    for i, model in enumerate(x_ticks):
        if model == 'full_model':
            ...
            plt.plot(model_errors, marker="^", ls='--')#, color='blue')

        elif model == 'adaboost':
            model = model + "/ensemble_model"
            model_errors = get_model_errors(directory_experiment,dataset,model)
            print('adaboost',model_errors)
            plt.plot(model_errors, marker="*", ls='--')#, color='orangered')
        else:
            # Plot of bagging only
            model = model + "/ensemble_model"
            model_errors = get_model_errors(directory_experiment,dataset,model)
            print('bagging',model_errors)
            plt.plot(model_errors, marker=markers[count], ls='--')
            count += 1
    ...
    for i, model in enumerate(x_ticks):
        if model == 'full_model':
            model_errors = get_model_errors_noise(directory_experiment_noise,dataset,model)
            print('full_model',model_errors)
            plt.plot(model_errors, marker="^", color=colors[count])
            count+=1
            ...
            model_errors = get_model_errors_noise(directory_experiment_noise,dataset,model)
            print('bagging',model_errors)
            plt.plot(model_errors, marker=markers[count], color=colors[count])
            count += 1
            
#    plt.ylim((0, 0.5))
    plt.ylabel('MSE')
    plt.xlabel('Layers')
    x_ticks = ['FM (ideal)', 'Bag_0.8_0.2 (ideal)', 'Bag_0.8_1.0 (ideal)', 'FM (noisy)', 'Bag_0.8_0.2 (noisy)', 'Bag_0.8_1.0 (noisy)']
    plt.legend(x_ticks, loc='upper center', handlelength=4)
    plt.xticks(ticks=range(len(model_errors)),labels=np.arange(1, len(model_errors)+1,1))

    #plt.xticks(ticks=range(len(x_ticks)), labels=x_ticks, rotation=-45)
    plt.tight_layout()
    #plt.grid()
    save_dir = f"{directory_experiment_noise}/plots_errors_layers/{dataset}"
    os.makedirs(save_dir,  0o755,  exist_ok=True)
    plt.savefig(save_dir + f"/{title}.png",dpi=600)
    plt.close('all')
...
def plot_error_layers(directory_experiment, dataset, title=f"Error of each model in terms of the number of layers"):
    ...
    x_ticks = ['full_model', 'bagging_feature03_sample02', 'bagging_feature03_sample10', 'bagging_feature05_sample02', 'bagging_feature05_sample10', 'bagging_feature08_sample02', 'bagging_feature08_sample10']
    markers = ['o', 'P', '8', 'x', 's', 'p']
    count = 0
    ...
    for i, model in enumerate(x_ticks):
        if model == 'full_model':
            model_errors = get_model_errors_noise(directory_experiment,dataset,model)
            print('full_model',model_errors)
...
#    plt.ylim((0, 0.5))
    plt.ylabel('MSE')
    plt.xlabel('Layers')
    x_ticks = ['FM', 'Bag_0.3_0.2', 'Bag_0.3_1.0', 'Bag_0.5_0.2', 'Bag_0.5_1.0', 'Bag_0.8_0.2', 'Bag_0.8_1.0']

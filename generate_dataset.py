...
def concrete_dataset_c(scaled=False):
    # data were taken from: https://code.datasciencedojo.com/datasciencedojo/datasets/tree/master/Concrete%20Compressive%20Strength
    data = pd.read_excel('concrete_data.xls')
    data = np.array(data)
    X = data[:,0:-1]
    y = data[:,-1]
    directory = 'datasets/concrete'
    os.makedirs(directory, exist_ok=True)
    np.save(f"{directory}/X.npy", X)
    np.save(f"{directory}/y.npy", y)
